...
# Kept only as a last compatibility attempt.  Historical requests always try
# the documented 999-prefixed token first.
...
def _historical_rows_with_index_tokens(obj, params):
    ...
    for candidate_number, token in enumerate(candidates):
        # Retry the documented historical token.  The legacy token is only a
        # single final compatibility attempt, avoiding excessive API traffic.
        retry_delays = (
        ...
        for delay in retry_delays:
            ...
            with live_patch._ANGEL_HISTORY_LOCK:
                elapsed = (
                    time.monotonic()
                    - live_patch._ANGEL_LAST_HISTORY_REQUEST
                )
            ...
            if isinstance(response, dict):
                ...
                if response.get("status") is not False and not rows:
                    ...
                    last_error = (
                        f"API_ERROR token={token} message={message}"
                    )
...
def apply_angel_historical_index_patch():
    ...
    # routes imported a separate dictionary snapshot from angel_fetcher.  Only
    # that backtest snapshot is replaced; live feed constants remain unchanged.
    routes.INDEX_TOKENS = {
