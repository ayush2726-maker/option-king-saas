"""Balanced exit, 4% cost-safe breakeven, and softer cooldown patch.

This patch is intentionally exit/risk-side only:
- Entry strategy and score rules are left unchanged.
- AI output is not used to allow/block trades here.
- Bought CE and bought PE are both managed by option premium LTP.
"""
...
LOCK_075_TRIGGER_R = 1.50
LOCK_125_TRIGGER_R = 2.00
DYNAMIC_TRAIL_TRIGGER_R = 2.50
...
def _ist_day_bounds_utc(now: datetime | None = None) -> tuple[datetime, datetime]:
...
def _balanced_profit_lock(
    ...
    """Profit lock chosen by Ayush: 4% BE, late locking, SL never widens."""
    ...
    # Do not tighten before +0.8R. At +0.8R, first lock is cost-safe
    # entry + charges + 4% net premium profit. If the premium has not created
    # enough room for this price, keep old SL instead of placing SL above LTP.
...
def _early_danger_exit(trade, ltp, market_data, trail):
    """Exit before full SL only when option is failing and index confirms danger."""
    if not market_data:
    ...
    danger = vwap_break and ema_break and trend_against
    if not danger:
        return {"exit": False, "reason": None}
    ...
    return {
        ...
        "near_sl": bool(near_sl),
        "vwap_break": bool(vwap_break),
        "ema_break": bool(ema_break),
        "trend_against": bool(trend_against),
    }
...
def _legacy_dynamic_manage_open(conn, trade, ltp, market_data=None, candle_id=None):
    entry = _f(trade["entry_price"])
    qty = max(1, _i(trade["qty"], 1))
    old_sl = _f(legacy._dv(trade, "sl_price", max(0.05, entry - 0.05)))
    risk = _f(legacy._dv(trade, "initial_risk", max(0.05, entry - old_sl)))
    peak = _f(legacy._dv(trade, "peak_price", entry))
    updates = _i(legacy._dv(trade, "trail_updates", 0))
    ...
    conn.execute(
        """UPDATE paper_trades
           SET sl_price=?, target_price=NULL, initial_risk=?,
               ...
               last_ltp=?, reversal_count=?, reversal_last_candle=?
           WHERE id=?""",
        (
            trail["sl_price"], risk, trail["peak_price"], trail["stage"], updates,
            ltp, reversal["count"], reversal["last_candle"], trade["id"],
        ),
    )
    ...
    hit = ltp <= trail["sl_price"]
    if not hit and not danger.get("exit") and not reversal["exit"] and not eod:
        conn.commit()
        return
    ...
    conn.execute(
        """UPDATE paper_trades
           SET exit_price=?, pnl=?, status='CLOSED', reason=?, last_ltp=?
           WHERE id=?""",
...
def _risk_exit_text(reason: str) -> bool:
...
def apply_balanced_exit_cooldown_patch():
    if getattr(runtime, "_okai_balanced_exit_cooldown_v3", False):
        return
    ...
    legacy._dynamic_manage_open = _legacy_dynamic_manage_open
