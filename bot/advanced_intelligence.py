"""Railway broker-neutral advanced AI and exact option outcome monitor.
...
from bot.adaptive_model import feature_vector, maybe_train_models, model_status, predict_adaptive
...
VERSION = "OKAI-ADVANCED-BROKER-NEUTRAL-SHADOW-V1"
HORIZONS = (5, 15, 30)
PRIMARY_HORIZON = 15
POLL_SECONDS = 15
...
def ensure_advanced_schema() -> None:
    ...
    try:
        conn.executescript("""
        CREATE TABLE IF NOT EXISTS ai_advanced_snapshots(
        ...
        CREATE INDEX IF NOT EXISTS idx_ai_advanced_user_created
          ON ai_advanced_snapshots(user_id,created_at DESC);
        CREATE INDEX IF NOT EXISTS idx_ai_advanced_pending
          ON ai_advanced_snapshots(user_id,complete,created_at);
        CREATE TABLE IF NOT EXISTS ai_advanced_contract_outcomes(
        ...
        CREATE INDEX IF NOT EXISTS idx_ai_advanced_outcome_user_horizon
          ON ai_advanced_contract_outcomes(user_id,horizon_minutes,evaluated_at DESC);
        ...
        """)
        now = _iso()
        conn.execute("""INSERT INTO ai_advanced_runtime VALUES(1,?,?,?,?,NULL,0,0)
...
def _previous_oi(user_id: int) -> Dict[str, float]:
    conn = get_db()
    try:
        row = conn.execute("""SELECT option_summary_json FROM ai_advanced_snapshots
...
def register_snapshot(user_id, market, base, option_payload, news, advanced):
    ...
    conn = get_db()
    try:
        last = conn.execute("SELECT created_at,advanced_decision,broker,symbol FROM ai_advanced_snapshots WHERE user_id=? ORDER BY datetime(created_at) DESC,rowid DESC LIMIT 1", (user_id,)).fetchone()
        if last:
        ...
        conn.commit()
        print(f"AI ADVANCED RAILWAY | logged | user={user_id} | broker={option_payload.get('broker')} | {advanced.get('decision')} {advanced.get('confidence')}% | blocking OFF")
        return decision_id
    finally:
        conn.close()
...
def observe_outcomes(user_id, market, option_payload):
    ...
    created = 0
    try:
        pending = conn.execute("SELECT * FROM ai_advanced_snapshots WHERE user_id=? AND complete=0 AND symbol=? ORDER BY datetime(created_at)", (user_id, str(market.get("symbol") or "NIFTY"))).fetchall()
        for row in pending:
            ...
            for horizon in HORIZONS:
                if elapsed < horizon * 60:
                    continue
                if conn.execute("SELECT 1 FROM ai_advanced_contract_outcomes WHERE decision_id=? AND horizon_minutes=?", (row["id"], horizon)).fetchone():
                    continue
                ...
                conn.execute("""INSERT OR IGNORE INTO ai_advanced_contract_outcomes VALUES(
                NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
                ...
                created += 1
            count = conn.execute("SELECT COUNT(*) n FROM ai_advanced_contract_outcomes WHERE decision_id=?", (row["id"],)).fetchone()["n"]
            if _i(count) >= len(HORIZONS):
                conn.execute("UPDATE ai_advanced_snapshots SET complete=1,completed_at=? WHERE id=?", (_iso(), row["id"]))
        conn.commit()
        return created
    finally:
        conn.close()
...
def _calibration(user_id):
    conn = get_db()
    try:
        rows = conn.execute("""SELECT s.advanced_confidence,o.advanced_outcome FROM ai_advanced_snapshots s
        JOIN ai_advanced_contract_outcomes o ON o.decision_id=s.id WHERE s.user_id=? AND o.horizon_minutes=?""", (user_id, PRIMARY_HORIZON)).fetchall()
    finally:
        conn.close()
    buckets = {}
    for row in rows:
...
def get_advanced_summary(user_id, recent_limit=20):
    ...
    conn = get_db()
    try:
        rows = conn.execute("SELECT * FROM ai_advanced_snapshots WHERE user_id=? ORDER BY datetime(created_at) DESC,rowid DESC LIMIT ?", (user_id, max(1, min(_i(recent_limit,20),50)))).fetchall()
        primary = conn.execute("SELECT * FROM ai_advanced_contract_outcomes WHERE user_id=? AND horizon_minutes=?", (user_id, PRIMARY_HORIZON)).fetchall()
        recent = []
        ...
        for row in rows:
            ...
            outcomes = []
            for outcome in conn.execute("SELECT * FROM ai_advanced_contract_outcomes WHERE decision_id=? ORDER BY horizon_minutes", (item["id"],)).fetchall():
...
def advanced_health():
    ...
    conn = get_db()
    try:
        runtime = conn.execute("SELECT * FROM ai_advanced_runtime WHERE singleton=1").fetchone()
        counts = conn.execute("SELECT COUNT(*) decisions,SUM(CASE WHEN complete=0 THEN 1 ELSE 0 END) pending FROM ai_advanced_snapshots").fetchone()
    finally:
        conn.close()
...
def _heartbeat(error=None):
    ...
    conn = get_db()
    try:
        conn.execute("UPDATE ai_advanced_runtime SET heartbeat_at=?,last_error=?,trade_blocking=0,order_execution=0 WHERE singleton=1", (_last_cycle,_last_error)); conn.commit()
    finally:
        conn.close()
...
def _cycle():
    ...
    for user_id in _users():
        try:
            ...
            print(f"AI ADVANCED RAILWAY | user={user_id} | {type(exc).__name__}:{str(exc)[:220]}")
...
def _loop():
    while True:
        ...
        try:
            ...
            print("AI ADVANCED RAILWAY | monitor warning | " + error)
        try:
            _heartbeat(error)
        except Exception as exc:
            print(f"AI ADVANCED RAILWAY | heartbeat warning | {str(exc)[:180]}")
...
def start_advanced_intelligence(snapshot_builder):
    ...
    with _lock:
        ...
        _thread = threading.Thread(target=_loop,name="okai-advanced-broker-neutral-shadow",daemon=True); _thread.start()
        print(f"AI ADVANCED RAILWAY {VERSION} active | Angel One + Upstox + Zerodha | exact option labels | blocking OFF | orders OFF")
        return advanced_health()
