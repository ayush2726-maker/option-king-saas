...
# Backward-compatible name used by the regression tests/older imports.
...
def _runtime_capital_size(
    ...
    """Size against the active PAPER cycle equity or LIVE broker capital."""
    ...
    affordability_lots = (
        int(math.floor(budget / one_lot_cost)) if one_lot_cost > 0 else 0
    )
    planned_risk = (
        max(0.05, _f(risk_points, 0.05)) if risk_points is not None else None
    )
    ...
    return {
        ...
        "max_planned_loss_percent": (
            NORMAL_MAX_PLANNED_LOSS_PERCENT if planned_risk is not None else None
        ),
        ...
        "planned_risk_per_lot": (
            round(planned_risk_per_lot, 2) if planned_risk_per_lot is not None else None
        ),
...
def _backtest_capital_size(capital, premium, lot_size, allocation):
    ...
    return {
        ...
        "slot_utilization_percent": (
            round(capital_used / max(0.01, budget) * 100.0, 2) if budget > 0 else 0.0
        ),
        "affordable": lots >= 1,
        "risk_cap_applied": lots < affordability_lots,
...
def _annotate_result(result):
    ...
    output["note"] = (
        "Quantity uses active capital-cycle equity and is additionally capped "
        "so planned stop loss is at most 10% of that sizing capital."
    )
    return output
...
def _install_current_capital_profile_fields():
    if getattr(user_panel_routes, "_okai_current_capital_profile_v2", False):
        return
    ...
    def user_profile_with_current_capital(authorization: str = Header(None)):
        ...
        try:
            ...
            sizing_capital = (
                _cycle_paper_sizing_base(conn, user_id, settings)
                if mode == "paper"
                else ledger.get("starting_capital")
            )
            current_capital = ledger.get("current_capital")
            if mode == "live" and current_capital is None:
                current_capital = ledger.get("broker_total_limit")
            ...
            profile.update({
                ...
                "paper_sizing_capital": sizing_capital if mode == "paper" else None,
                "sizing_capital": sizing_capital,
                "sizing_capital_source": (
                    "PAPER_RESET_CYCLE_EQUITY"
                    if mode == "paper"
                    else "LIVE_BROKER_CAPITAL"
                ),
                "broker_available_cash": ledger.get("broker_available_cash"),
                "broker_used_margin": ledger.get("broker_used_margin"),
                "broker_total_limit": ledger.get("broker_total_limit"),
                "broker_funds_updated_at": ledger.get("broker_funds_updated_at"),
...
def _install_capital_change_resets_cycle():
    if getattr(strategy_routes, "_okai_paper_capital_cycle_reset_v1", False):
        return

    original = strategy_routes.save_settings

    def save_settings_with_capital_cycle_reset(body: dict, authorization: str = Header(None)):
        user = strategy_routes.get_current_user(authorization)
        conn = get_db()
        try:
            old_settings = strategy_routes.ensure_settings(conn, int(user["id"]))
            old_capital = _f(old_settings.get("paper_capital", 100000), 100000)
        finally:
            conn.close()
        ...
        saved = dict(result.get("settings") or {})
        new_capital = _f(saved.get("paper_capital", old_capital), old_capital)
        if abs(new_capital - old_capital) <= 1e-9:
            return result

        now = datetime.utcnow().isoformat()
        saved[RESET_KEY] = now
        saved[CARRY_KEY] = True

        conn = get_db()
        try:
            conn.execute(
                "INSERT OR REPLACE INTO strategy_settings "
                "(user_id, settings_json, updated_at) VALUES (?, ?, ?)",
                (int(user["id"]), strategy_routes.json.dumps(saved), now),
            )
            conn.commit()
        finally:
            conn.close()
        ...
        result["paper_capital_cycle_reset"] = True
        result["paper_capital_reset_at"] = now
        result["message"] = "Strategy settings saved; PAPER capital cycle restarted"
        return result
...
def apply_capital_based_sizing_restore_patch():
    """Install final runtime sizing, capital-cycle reset, and capital display fields."""
    runtime._size = _runtime_capital_size
    runtime._paper_base = _cycle_paper_sizing_base
    ...
    _install_capital_change_resets_cycle()
