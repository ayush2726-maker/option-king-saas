...
def feature_vector(market, base, news, global_data, options):
    ...
    return {
        ...
        "institutional_bias": num(options.get("institutional_bias_score")) / 100,
    }
...
def fuse(market, base, news, global_data, options, registry):
    ...
    institutional = num(options.get("institutional_bias_score"))
    if abs(institutional) >= 15:
        institutional_side = "CE" if institutional > 0 else "PE"
        scores[institutional_side] += clamp(abs(institutional) * .14, 3, 14)
        reasons.append("INSTITUTIONAL_FLOW_" + institutional_side)
