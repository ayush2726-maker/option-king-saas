...
VERSION = "OKAI-INDEX-REPORT-CARD-V1"
...
def _mode(row: Any) -> str:
    return (
        "live"
        if str(_value(row, "trading_mode", "paper") or "paper").lower()
        == "live"
        else "paper"
    )
...
def _closed_pnl(row: Any) -> float:
    ...
    return round(
        _number(
            net if net is not None else _value(row, "pnl", 0),
            0,
        ),
        2,
    )
...
def _finalize(summary: dict) -> dict:
    ...
    summary["average_pnl"] = (
        round(summary["realized_pnl"] / closed, 2)
        if closed
        else 0.0
    )
    summary["best_trade"] = (
        round(float(summary["best_trade"]), 2)
        if summary["best_trade"] is not None
        else None
    )
    summary["worst_trade"] = (
        round(float(summary["worst_trade"]), 2)
        if summary["worst_trade"] is not None
        else None
    )
    summary["no_loss_trade"] = bool(closed and gross_loss == 0)
    summary["profit_factor"] = (
        round(gross_profit / abs(gross_loss), 2)
        if gross_loss < 0
        else None
    )
    return summary
...
def build_index_report_card(
    ...
    summaries = {
        instrument: _empty_summary(instrument)
    ...
    for row in rows:
        ...
        summary["best_trade"] = (
            ...
            else max(float(summary["best_trade"]), pnl)
        )
        summary["worst_trade"] = (
            pnl
            if summary["worst_trade"] is None
            else min(float(summary["worst_trade"]), pnl)
        )
        if pnl > 0:
    ...
    results = [
        _finalize(summaries[instrument])
        for instrument in INSTRUMENTS
    ]
    ranked = sorted(
    ...
    total_realized = round(
        sum(item["realized_pnl"] for item in results),
        2,
    )
    ...
    return {
        "version": VERSION,
        "source": "PAPER_TRADES_DB_ALL_TIME_NET_PNL",
        ...
        "closed_trades": sum(
            item["closed_trades"] for item in results
        ),
        "open_trades": sum(
            item["open_trades"] for item in results
        ),
