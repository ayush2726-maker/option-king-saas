...
def _paper_summary(conn, user_id: int, settings: dict) -> dict:
    seed = max(1.0, _f(settings.get("paper_capital", 100000), 100000)); reset_at = str(settings.get(RESET_KEY) or "").strip()
    pnl_expression = "COALESCE(net_pnl, pnl, 0)" if _has_column(conn, "paper_trades", "net_pnl") else "COALESCE(pnl, 0)"
    ...
    row = conn.execute(f"SELECT COALESCE(SUM({pnl_expression}), 0) AS net_pnl, COUNT(*) AS closed_trades FROM paper_trades WHERE {' AND '.join(where)}", tuple(params)).fetchone()
    cumulative_net_pnl = round(_f(row["net_pnl"] if row else 0), 2); equity = max(1.0, round(seed + cumulative_net_pnl, 2))
    return {"seed_capital": round(seed, 2), "cumulative_net_pnl": cumulative_net_pnl, "equity": equity, "closed_trades": int(row["closed_trades"] if row else 0), "reset_at": reset_at or None}
...
def _reset_continuous_paper_account(body: dict = None, authorization: str = Header(None)):
    ...
    try:
        ...
        try: conn.execute("UPDATE bot_status SET total_pnl=0, total_trades=0, last_signal='PAPER_RESET', updated_at=? WHERE user_id=?", (now, int(user["id"]))); conn.commit()
        except Exception: pass
    finally: conn.close()
    try: paper_routes.notify_user(int(user["id"]), f"♻️ <b>Paper Capital Reset</b>\nCapital: ₹{capital:,.0f}\nNaya sizing cycle isi amount se start hua; purana P&L history me rahega.")
    except Exception: pass
...
def apply_capital_continuity_patch() -> None:
    ...
    runtime._paper_base = _continuous_paper_base; runtime._okai_paper_capital_carry_forward_v1 = True; runtime._okai_paper_sizing_source = "PAPER_RESET_CYCLE_EQUITY"; runtime._okai_live_capital_source = "BROKER_AVAILABLE_FUNDS"
    paper_routes.paper_account = _continuous_paper_account; paper_routes.reset_paper_account = _reset_continuous_paper_account
    _replace_route(paper_routes.router, "/paper/account", "GET", _continuous_paper_account); _replace_route(paper_routes.router, "/paper/reset", "POST", _reset_continuous_paper_account); _INSTALLED = True
