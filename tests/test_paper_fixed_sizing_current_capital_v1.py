...
def _closed(c,p,t):
    c.execute("INSERT INTO paper_trades(user_id,status,pnl,net_pnl,trading_mode,entry_price,last_ltp,qty,created_at,exit_time) VALUES(1,'CLOSED',?,?,'paper',100,100,1,?,?)",(p,p,t,t));c.commit()

def test_profit_after_reset_increases_next_paper_sizing_base():
    c=_conn();_closed(c,5000,"2026-09-02 10:00:00");s={"trading_mode":"paper","paper_capital":20000,"paper_capital_reset_at":"2026-09-02 09:00:00"};assert _configured_paper_sizing_base(c,1,s)==25000;l=build_authoritative_ledger(c,1,s);assert l["starting_capital"]==20000 and l["current_capital"]==25000;c.close()
...
def test_runtime_quantity_uses_cycle_equity():
    a=_runtime_capital_size(20000,1,90,65,[],None);b=_runtime_capital_size(25000,1,90,65,[],None);assert a["qty"]==65 and b["qty"]==130

def test_live_current_capital_uses_fresh_gateway_funds_when_flat():
    c=_conn();c.execute("CREATE TABLE live_broker_funds(user_id INTEGER PRIMARY KEY,available_cash REAL,used_margin REAL,total_limit REAL,broker TEXT,updated_at TEXT)");c.execute("INSERT INTO live_broker_funds VALUES(1,18000,2000,20000,'angelone',datetime('now'))");c.commit();l=build_authoritative_ledger(c,1,{"trading_mode":"live","paper_capital":999999});assert l["starting_capital"]==20000 and l["current_capital"]==20000 and l["broker_available_cash"]==18000 and l["broker_total_limit"]==20000;c.close()
